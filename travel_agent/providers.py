...
DEFAULT_AMADEUS_CONFIG_PATH = Path(__file__).with_name("amadeus_config.json")
...
class AmadeusConfig:
    ...
    def from_file(
        ...
        """Build a configuration from a JSON file containing Amadeus credentials.

        Parameters
        ----------
        path:
            Optional path to a JSON file with the Amadeus credentials. Defaults
            to the ``amadeus_config.json`` file that ships with the package.
        """

        config_path = Path(path) if path is not None else DEFAULT_AMADEUS_CONFIG_PATH
        try:
        ...
        client_id = str(payload.get("client_id")) if payload.get("client_id") else ""
        client_secret = str(payload.get("client_secret")) if payload.get("client_secret") else ""
        missing: list[str] = []
        if not client_id:
        ...
        if missing:
            raise ValueError(
                "Missing required Amadeus credentials in configuration file: "
                + ", ".join(missing)
            )

        hostname = str(payload.get("hostname")) if payload.get("hostname") else "https://test.api.amadeus.com"
